...
class BookingGymBloc extends Bloc<BookingGymEvent, BookingGymState> {
  ...
  _onBookingSubmitted(
      BookingSubmitted event, Emitter<BookingGymState> emit) async {
    //*State menjadi submitting
    emit(
        state.copyWith(idMember: event.idMember, formStatus: FormSubmitting()));
    try {
      //*request login
      final response = await bookingGymRepo.store(
          IdMember: state.idMember,
          bookingDate: state.dateBooking,
          idSesi: state.sesi);
